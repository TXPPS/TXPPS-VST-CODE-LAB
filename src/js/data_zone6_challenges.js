...
const ZONE6_CHALLENGES = [
  ...
  /* ============ BOSS ============ */
  {
    id: 'boss6', kind: 'boss', title: 'BOSS: Production Engineer', short: 'Zone 6 boss', passNeed: 4,
    concepts: ['code-review', 'defensive-programming', 'regression-testing', 'assertions', 'memory-diagnostics', 'release-readiness'],
    brief: 'First Signal 1.0-rc1 works — but "works" is not "ships". Six open findings stand between the candidate and sign-off: an ownership smell in review, an unsafe host assumption, an unverified DSP refactor, a missing tripwire, a memory leak, and an unordered verification pipeline. Close at least 4 findings, on evidence, to sign off the release.',
    stages: [
      {
        type: 'bugspot', concept: 'code-review', qid: 'b6s1',
        prompt: 'Phase 1 — Engineering Review. This diff wants to merge into the release branch. One line reintroduces a class of bug Zone 2 retired. Tap it.',
        code: [
          'class PresetManager',
          '{',
          'public:',
          ...
          'private:',
          '    Preset* current = new Preset();   // TODO: delete somewhere?',
          '    std::vector<Preset> history;',
          '};',
        ],
        buggy: 5,
        explain: 'A raw owning `new` with a TODO where the release should be — review rejects this on sight. Zone 2\'s rule, applied at review speed: every resource has one owner, and the owner is visible in the type. Here that means `std::unique_ptr<Preset>` — or better, a plain `Preset` member: it never needed the heap at all. The memory-leak bug the team fixed in rb2 started life as exactly this line.',
        fix: 'std::unique_ptr<Preset> current;   // or a plain Preset member',
      },
      {
        type: 'mcq', concept: 'defensive-programming', qid: 'b6s2',
        prompt: 'Phase 1 — Engineering Review. This passed every bench test, then glitched in one specific DAW. Name the unsafe assumption.',
        code: 'void prepareToPlay(double sr, int samplesPerBlock)\n{\n    scratch.resize(samplesPerBlock);\n}\nvoid processBlock(juce::AudioBuffer<float>& b, juce::MidiBuffer&)\n{\n    for (int i = 0; i < (int) scratch.size(); ++i)\n        b.getWritePointer(0)[i] += scratch[i];\n}',
        options: [
          { t: 'It assumes every callback delivers exactly samplesPerBlock samples — that number is a maximum hint, and hosts may send fewer (and varying) samples. Loop over buffer.getNumSamples().', why: '' },
          { t: 'resize() in prepareToPlay is an allocation violation', why: 'prepareToPlay is exactly where allocation belongs — Zone 5\'s contract. The defect is the loop bound in processBlock, not the setup.' },
          { t: 'scratch should live on the stack instead', why: 'Its size depends on host settings, so a pre-allocated member is the right home. Where it lives is fine; how far the loop runs is not.' },
          { t: 'Nothing — hosts always deliver full-size blocks', why: 'Mainstream hosts split blocks at automation points and loop boundaries. That is why processBlock hands you the real count every call — the API is telling you not to assume.' },
        ],
        answer: 0,
        explain: 'prepareToPlay\'s samplesPerBlock is an upper bound, not a promise: the host may deliver fewer samples, and the count can change call to call — splitting at automation points and loop edges is normal. Looping to scratch.size() walks past the buffer\'s valid samples in exactly the hosts your bench never runs. The prepare contract (Zone 4) says: size by the maximum, but process by buffer.getNumSamples(), every block.',
      },
      {
        type: 'mcq', concept: 'regression-testing', qid: 'b6s3',
        prompt: 'Phase 2 — Verification. A refactor rewrote the filter\'s inner loop and the author says "no audible change". What closes this finding?',
        code: '// PR #214: "optimised biquad inner loop — no audible change"\n// reviewer: prove it.',
        options: [
          { t: 'A golden-render regression test: render fixed input through old and new builds and diff the output sample-by-sample — evidence, not ears', why: '' },
          { t: 'Listen carefully on good monitors', why: 'Ears adapt, fatigue and forgive; a fraction-of-a-dB drift or a one-sample discontinuity slides straight past monitoring. Zone 3 taught what even tiny steps become: audible artifacts.' },
          { t: 'Read the diff twice, carefully', why: 'Review catches design problems; it cannot verify numeric equivalence of DSP output. Only rendering both versions and comparing can.' },
          { t: 'Ship it labelled "beta" and wait for reports', why: 'That outsources your regression testing to paying customers — the most expensive test rig ever invented.' },
        ],
        answer: 0,
        explain: 'The golden render (ro1, promoted to a reflex): render a deterministic input through both builds and compare within tolerance. Identical in, identical out — any unexpected delta is a regression, caught in minutes and attached to the PR as evidence. Zone 3\'s signal-integrity lesson is why the tolerance is tight: discontinuities you cannot hear on a meter are still broadband artifacts. Claims about sound are verified by rendering sound.',
      },
      {
        type: 'fill', concept: 'assertions', qid: 'b6s4',
        prompt: 'Phase 2 — Verification. Document the invariant so development builds trip on it instantly — and release builds pay nothing. Complete the tripwire.',
        code: 'void setRouting(int numChannels)\n{\n    ___ (numChannels <= maxChannels);   // fires in debug, free in release\n    routing.applyTo(numChannels);\n}',
        accept: ['jassert'],
        placeholder: 'macro',
        hint: 'JUCE\'s debug tripwire — rc4 installed one. Lowercase, starts with j.',
        mistakes: [
          { match: '^assert$', msg: 'The C assert exists, but JUCE code uses its portable, debugger-halting cousin — same idea, j-prefixed.' },
          { match: '^static_assert$', msg: 'static_assert checks facts the COMPILER can know. numChannels arrives at runtime — you need the runtime tripwire.' },
          { match: '^jassertfalse$', msg: 'jassertfalse fires unconditionally — it marks code that should never RUN. Here you want the conditional form that checks the invariant.' },
        ],
        explain: 'jassert documents the invariant where it lives, halts the debugger the instant development code breaks it, and compiles to nothing in release — the tripwire costs zero where Zone 5\'s deadline rules (an assertion that could stall the audio thread in production would be a bug of its own). Assertions catch what you anticipated; pair them with logging and diagnostics for what you did not.',
      },
      {
        type: 'mcq', concept: 'memory-diagnostics', qid: 'b6s5',
        prompt: 'Phase 3 — Release Candidate. Finding: RAM climbs ~40 MB on every preset load and never comes back; CPU stays flat. Which instrument closes this finding?',
        code: '// bug tracker #88 — "memory grows with every preset load"\n// CPU: normal.  Audio: clean.  RAM: staircase, up only.',
        options: [
          { t: 'A heap diagnostic (leak sanitizer / allocation snapshots): diff allocations across a load→unload cycle — the surviving blocks\' stacks name the owner that never releases', why: '' },
          { t: 'A CPU profiler', why: 'Profilers measure where TIME goes, and the report says CPU is flat. A staircase in RAM is an ownership question, and ownership needs a memory instrument.' },
          { t: 'More jasserts around the preset code', why: 'Assertions verify invariants you anticipated; a leak is memory you forgot exists. You cannot assert your way to an allocation you are not tracking — measure instead.' },
          { t: 'Advise users to restart between sessions', why: 'That hides the evidence and ships the bug. The staircase is data — a diagnostic turns it into a line number.' },
        ],
        answer: 0,
        explain: 'Leak triage is measurement: snapshot the heap, run one load→unload cycle, snapshot again — every surviving allocation is a suspect, and its stack trace names the owner that never released. Then the fix is Zone 2 verbatim: give the resource one owner whose destructor runs (the staircase means someone holds `new` results with no release path — rb2\'s bug, found the professional way). Diagnosis by instrument, repair by ownership.',
      },
      {
        type: 'order', concept: 'release-readiness', qid: 'b6s6',
        prompt: 'Phase 3 — Release Candidate. Final gate. Order the sign-off pipeline so the cheapest evidence runs first and every gate must pass before the next spends more.',
        lines: [
          'if (! runUnitTests())       return KEEP_IN_REVIEW;   // seconds — the pieces, proven in isolation',
          'if (! runGoldenRenders())   return KEEP_IN_REVIEW;   // minutes — the sound itself, diffed sample-by-sample',
          'if (! runHostValidation())  return KEEP_IN_REVIEW;   // hours — pluginval + real DAWs prove the whole contract',
        ],
        explain: 'Cheapest evidence first: units fail in seconds, golden renders in minutes, host validation in hours — ordering the gates this way is what makes running them EVERY time affordable, and running them every time is what "release candidate" means. The sign-off this pipeline earns rests on every zone you have cleared: ownership clean (Zone 2), renders null (Zone 3), the prepare contract held (Zone 4), the deadline safe under load (Zone 5). Not code that works — code you can prove works. Sign-off granted: Zone 6 is yours.',
      },
    ],
  },
];

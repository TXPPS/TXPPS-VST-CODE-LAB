...
const ZONE2_CHALLENGES = [
  ...
  /* ============ BOSS ============ */
  {
    id: 'boss2', kind: 'boss', title: 'BOSS: Ownership Crisis', short: 'Zone 2 boss', passNeed: 4,
    concepts: ['object-lifetime', 'raii', 'smart-pointers', 'ownership-transfer', 'double-ownership'],
    brief: 'A collaborator\'s synth has a corrupting ownership graph: objects allocated with no clear owner, resources that never release, pointers that outlive what they point to. Trace every object back to exactly one owner. Six repairs, one retry each, contain at least 4 to stabilise it.',
    stages: [
      {
        type: 'mcq', concept: 'object-lifetime', qid: 'b2s1',
        prompt: 'Phase 1 — Three objects, one has no owner watching it. When renderVoice() returns, which one leaks?',
        code: 'void renderVoice() {\n    Envelope env;                        // 1\n    auto lfo = std::make_unique<LFO>();  // 2\n    Filter* filt = new Filter();         // 3\n}                                        // scope ends here',
        options: [
          { t: '(3) filt — a raw `new` with no `delete`. The pointer dies with the scope; the heap Filter is orphaned.', why: '' },
          { t: '(1) env — it lives on the stack', why: 'Stack objects are destroyed automatically at the closing brace — env\'s destructor runs on the way out. Nothing leaks there.' },
          { t: '(2) lfo — make_unique put it on the heap', why: 'It is on the heap, but unique_ptr owns it: when lfo leaves scope its destructor deletes the LFO. That is exactly what make_unique buys you.' },
          { t: 'None — the compiler frees all three', why: 'The compiler only cleans up stack objects and RAII owners. A bare `new` with no matching `delete` is orphaned heap memory — nothing collects it for you.' },
        ],
        answer: 0,
        explain: 'Stack objects (env) and RAII owners (lfo) clean themselves up at scope end; a bare `new` (filt) has no owner, so it leaks. Modern fix: `auto filt = std::make_unique<Filter>();` — now it has one owner and an automatic release.',
      },
      {
        type: 'bugspot', concept: 'object-lifetime', qid: 'b2s2',
        prompt: 'Phase 1 — This hands the caller a pointer to something that will not survive. Tap the line that dangles.',
        code: [
          'Preset* currentPreset()',
          '{',
          '    Preset p = loadDefault();',
          '    return &p;',
          '}',
        ],
        buggy: 3,
        explain: '`&p` is the address of a local. p\'s lifetime ends at the closing brace, so the caller gets a pointer to destroyed memory — a dangling pointer, undefined behavior. Return by value and let move / RVO carry it out safely.',
        fix: 'Preset currentPreset() { ...; return p; }   // return by value',
      },
      {
        type: 'compiler', concept: 'ownership-transfer', qid: 'b2s3',
        prompt: 'Phase 2 — Hand the filter from `a` to `b`. It will not build. What is the fix?',
        code: 'std::unique_ptr<Filter> a = std::make_unique<Filter>();\nstd::unique_ptr<Filter> b = a;   // give ownership to b',
        error: "ownership.cpp:2:31: error: use of deleted function\n'std::unique_ptr<Filter>::unique_ptr(const unique_ptr&)'",
        options: [
          { t: '`std::unique_ptr<Filter> b = std::move(a);`', why: '' },
          { t: '`std::unique_ptr<Filter> b = a.get();`', why: '`get()` copies out the raw pointer without transferring ownership — now `a` and the raw pointer both think they manage the Filter. That is double ownership and a double delete waiting to happen.' },
          { t: 'Make `Filter` copyable', why: 'The error is not about Filter. unique_ptr forbids copying on purpose — two owners of one resource is the bug it prevents. Move, do not copy.' },
          { t: 'Switch both to `std::shared_ptr`', why: 'That changes the design to shared ownership when you only need to hand it over once. For a one-time transfer, `std::move` is the precise tool.' },
        ],
        answer: 0,
        explain: 'unique_ptr is non-copyable by design: one resource, one owner. `std::move(a)` transfers ownership — afterward `b` owns the Filter and `a` is empty (null). The resource is still deleted exactly once, when `b` dies.',
      },
      {
        type: 'fill', concept: 'raii', qid: 'b2s4',
        prompt: 'Phase 2 — This guard opens a file but leaks the handle on every early return. Give it the member that releases automatically.',
        code: 'class FileGuard {\npublic:\n    FileGuard(const char* path) { handle = openFile(path); }\n    ___()                       { closeFile(handle); }\nprivate:\n    FileHandle handle;\n};',
        accept: ['~FileGuard'],
        placeholder: 'name',
        hint: 'The constructor\'s counterpart — runs automatically when the guard leaves scope.',
        mistakes: [
          { match: '^FileGuard$', msg: 'Missing the tilde — a destructor is written `~FileGuard`, the power-down twin of the constructor.' },
          { match: 'delete', msg: 'No manual delete needed. Name the destructor `~FileGuard`; wiring closeFile into it releases on every exit path.' },
        ],
        explain: '`~FileGuard()`: with closeFile wired into the destructor, every exit — normal return, early return, or exception — releases the file. That is RAII: the resource\'s lifetime is tied to an object\'s, so cleanup cannot be forgotten.',
      },
      {
        type: 'mcq', concept: 'double-ownership', qid: 'b2s5',
        prompt: 'Phase 3 — Both pools store the same raw `Voice*`, and each deletes its voices on teardown. What happens?',
        code: 'Voice* v = new Voice();\npoolA.push_back(v);   // pool A holds the raw pointer\npoolB.push_back(v);   // pool B holds the SAME raw pointer\n// teardown: each pool deletes the voices it holds',
        options: [
          { t: 'Double delete — the Voice is freed twice. Undefined behavior: crash or heap corruption.', why: '' },
          { t: 'Nothing — deleting the same pointer twice is harmless', why: 'The second delete operates on already-freed memory: undefined behavior. It may crash now, corrupt the heap, or rot silently until it crashes somewhere else.' },
          { t: 'A memory leak — the Voice is never freed', why: 'The opposite problem — it is freed too many times, not too few. Two owners each claim responsibility for one resource.' },
          { t: 'The compiler rejects the second push_back', why: 'Copying a raw pointer is perfectly legal — which is exactly why raw pointers cannot express ownership. The compiler cannot see the intent.' },
        ],
        answer: 0,
        explain: 'Two owners, one resource → double delete → undefined behavior. If both pools truly share the voice, say so: `std::shared_ptr<Voice>` frees it once, when the last owner leaves. If only one should own it, keep a single `unique_ptr` and let the other pool hold a non-owning (borrowed) pointer.',
      },
      {
        type: 'predict', concept: 'smart-pointers', qid: 'b2s6',
        prompt: 'Phase 3 — Final repair. Two Tracks hold a shared_ptr to each other, then both go out of scope. What happens, and what fixes it?',
        code: 'struct Track {\n    std::shared_ptr<Track> link;   // points to another Track\n};\n// a->link = b;   b->link = a;   // the two Tracks now reference each other',
        options: [
          { t: 'Neither is freed — each keeps the other\'s reference count at 1. Break the cycle: make one edge a `std::weak_ptr<Track>`.', why: '' },
          { t: 'Both are freed normally — shared_ptr always cleans up', why: 'shared_ptr frees when its count reaches zero, but here each object holds the other alive, so neither count ever hits zero. A reference cycle: a leak smart pointers alone cannot solve.' },
          { t: 'Double delete — they free each other twice', why: 'The opposite: nothing is freed. The mutual shared_ptrs pin both counts at 1 forever.' },
          { t: 'It will not compile — shared_ptrs cannot reference each other', why: 'It compiles fine — which is what makes the leak sneaky. The fix is to make the non-owning edge a `weak_ptr`, which does not raise the count.' },
        ],
        answer: 0,
        explain: 'A shared_ptr cycle is a leak: the mutual references keep each refcount above zero, so neither destructor ever runs. Decide which edge is ownership and which is a back-reference, and make the back-reference a `std::weak_ptr` — it observes without owning. The cycle breaks and both objects free. Ownership graph stabilised: Zone 2 cleared.',
      },
    ],
  },
];

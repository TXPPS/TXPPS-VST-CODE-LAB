...
const ZONE4_LESSONS = [
    ...
    checks: [
      ...
      },
      {
        ...
        options: [
          ...
          { t: 'Style preference only', why: 'It\'s functional: integer division here quantizes every pitch to the nearest octave below. Every note becomes an A.' },
  ...
  /* ------------------------------------------------------ D5 */
  {
    ...
    sections: [
      {
        ...
        breakdown: [
          ['double phase', 'double, not float: it accumulates for hours (Zone 1\'s precision lesson)'],
      ...
      },
      {
        ...
        body: 'Past 2π, subtract 2π — **never reset to zero**. The overshoot fraction is real phase; discarding it detunes the oscillator by a tiny lie every single cycle, and tiny lies at 440 per second add up to audible drift. (You fixed exactly this in p2 — now you know its name: phase error.)',
        mistake: { code: 'if (phase >= twoPi)\n    phase = 0.0;        // ✗ throws away the overshoot\n\nif (phase >= twoPi)\n    phase -= twoPi;     // ✓ keeps the fraction', text: 'Subtract, don\'t reset. The waveform looks identical on a scope for one cycle — and drifts flat over minutes.' },
      },
    ],
    checks: [
      {
      ...
      },
      {
        ...
        options: [
          { t: 'Phase accumulates tiny steps for hours — float\'s precision drifts audibly; each output sample is used once', why: '' },
          ...
          { t: 'No reason; float is fine', why: 'Hours of accumulated float rounding = slow pitch drift. Real synths use double phase for real reasons.' },
        ],
        answer: 0,
        explain: 'Accumulators compound their errors; outputs don\'t. Double where errors accumulate, float where they don\'t — a rule you\'ll reuse constantly.',
      },
    ],
    recap: [
    ...
    analogyPanel: 'Phase is a tape loop\'s playhead position; the increment is the tape speed. Splice the loop badly (reset to zero) and every pass loses a millimeter — the pitch of the loop slowly lies.',
  ...
  /* ------------------------------------------------------ D6 */
  {
    ...
    sections: [
      ...
      },
      {
        ...
        body: 'The phase must advance once per *sample*, not once per channel — so the sample loop owns the advance, and the channel loop just copies the computed value. Swap the nesting carelessly and stereo runs the oscillator twice per sample: an octave up and wrong. Loop structure IS correctness here.',
      },
    ],
    checks: [
      {
        ...
        code: [
          ...
          '    for (int i = 0; i < buffer.getNumSamples(); ++i)',
          '    {',
          '        data[i] = (float) std::sin(phase) * 0.25f;',
          '        phase += phaseIncrement;',
          '    }',
          '}',
        ],
        buggy: 6,
        explain: 'With channels outside, the phase advances once per channel per sample — twice per sample in stereo, doubling the effective pitch (and desyncing channels). Generators put the sample loop outside and advance exactly once inside it.',
        fix: 'Sample loop outside; compute once; copy to every channel; advance once',
      },
      {
        type: 'fill', concept: 'oscillators',
  ...
  /* ------------------------------------------------------ D8 */
  {
    ...
    beginnerMistake: 'Reaching for 192 kHz sessions to "fix" aliasing from naive shapes. The shape still implies infinite harmonics — you moved the ceiling, not the problem. Band-limit the source.',
